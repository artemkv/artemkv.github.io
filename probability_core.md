...
- Prove causality between events by testing for independence, if confirmed, use Bayes rule for inference
...
- Law of large numbers is about value of Sn, the CLT is about its distribution
...
- My thought: this is a classical real world → math space → back to real world trip, expecting to land :D
...
- You can _standardize_ any general normal distribution N(mu, sigma^2) by subtracting mu and dividing by sigma: Z = (x - mu)/sigma
...
- My thought: this is the classical problem in neuroscience
...
- When 2 variables are fully dependent, i.e. Y is X, covariance is just a variance, Cov(X, X) = Var(X)
...
- My thought: this is another real world → math space → back to real world trip
...
### Convergence in probability (for reference)
...
- I.e. the probability that my random variable falls outside the certain band a converges to zero
