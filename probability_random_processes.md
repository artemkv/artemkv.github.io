...
- In this example, p is obviously not constant, as there are times of the day that you receive more trafic, but we ignore that
- We do that by saying "we will consider a fixed period of time during which the p is more or less constant"
- My thought: at some point you might question why all this mathematical vigor, if we already ignored so many factors, why not just using some "common sense" heuristics and then simply test the model to see if it's good enough?
- One way to think about this process is as of sequence of random variables X1, X2, ...
- At any moment of t (i.e. at any trial t), E[Xt] = p, Var(Xt) = p(1-p)
...
- 1) given amount of time, how many requests would arrive?
- 2) given number of requests, what time would it take for them to arrive?
...
- E[S] = n*p
- Var(S) = n*p(1-p)
- My thought: this is basically the formalized version of the "common sense" trafic estimation, something we did for USCM cost estimation, doesn't sound like a big "aha" moment
...
- E[Yk] = k/p
- Var(Yk) = k(1-p)/p^2
- So, knowing probability of receiving a request each second, we can predict how many second would pass at the moment we receive 1000 requests
- My thought: this sounds like period-frequency relation, does this have to be so complicated? Can't we just assume requests arriving at constant intervals, find out average interval length and multiply?
...
- 1) Start by indentifying possible states
...
- Example: if you predict the trajectory of a flying ball, the current position of the ball is not enough, you need the velocity too
...
- Some Markov chains never enter *steady state*, and sometimes initial state matters (for example, if some of the states are not reacheable from some other states)
...
- Give those conditions, if you take the recursive formula for calculating probability of a system to be in each of the states, then take the limit, you will get the recipe for building a system of equasions (*balance equasions*) that allow to find the steady state probabilities
- Those equasions end up being actually quite simple (the limit goes away)
- For example, if you have state A and state B, with transitions A->A, A->B, B->B and B->A, and probabilities P(A->A)=0.5, P(A->B)=0.5, P(B->B)=0.8 and P(B->A)=0.2, then the first equasion looks as follows:
- P(A) = P(A)*0.5 + P(B)*0.2, P(B) = P(A)*0.5 + P(B)*0.8, which gives 0.5*P(A)=0.2*P(B)
- To solve it, we need the second part of equasion: P(A)+P(B)=1
- The solution is trivial
- Of course, if you have hundreds of states, solving this system of equasions by hand is very tedious
...
- The probability to see the request completing depends of the state of the queue and the time it takes to process a request
- State Xn: number of customers in the queue
- If max size of the queue is 10, then we have 10 possible states
- An arrival of a requests moves the state from Xk to Xk+1, the completion of the request moves the state from Xk to Xk-1, the simultaneous arrival and completion keeps the system at the same state
