...
- Many different failures may be indistinguishable for participating nodes (client request is lost, timed out or the server crashed just after receiving the request)
...
- Some of the events may not be related using one of the three rules, so they cannot be ordered
...
- Consequently, the the state of a process is all of the events that have taken place on it
...
- You might decide to queue the out-of-order messages for a while, and then deliver them anyways (after a certain grace period) even if there is a gap in a sequence. But then, if the lower sequence number message somehow arrives, you would need to discard it
...
- Rules:
	- 1) if a message sent by P1 is delivered at P2, increment P2's local clock in the P1s position
...
- The condition T[Pk] <= local VC[Pk] for all k except k at P1 means "there is no messages missing from other processes"
...
- The causal delivery does not rule out total order anomalies: if 2 processes send messages roughly at the same time, then the 3rd process may deliver them in any order, depending which message arrive faster
...
- If Pi has already seen a marker message before, it stops recording on the channel Cki, sets Cki's final state as the sequence of all the incoming messages that arrived on Cki since recording began
...
- Two processes may start initiating snaphots independently at the same time, using the same marker messages, this is not a problem
...
- **Reliable delivery:** let P1 be a process that sends a message m to process P2. If neither P1 or P2 crashes, then P2 eventually delivers m
...
- But Bob needs to make sure Alize gets ack message
...
- Workaround: make a plan in advance, before splitting (formally known as "common knowledge")
...
- Proposal number is not a actual value that is being proposed, not to be confused, that number will come in phase 2
...
- The milestone 2 (**consesus**) is reached when the majority of acceptors have sent Accepted (n, val) message
- When proposer receives the majority of Accepted (n, val) messages, it knows the consesus has been reached
...
- Our hope is, this will happen rarely
...
- Overall, Paxus falls in a category of algorithms that trade liveness for safety
...
- **Zab (ZooKeeper atomic broadcast)**. Comes from Yahoo, late 2000s, part of ZooKeeper. ZooKeeper is opensource, so you can use it
...
- As network partitions are fact of life, you are essentially chosing between consistency and availability (CAP theorem)
...
- If you want to test how the system behaves in face of errors, you need to intentionally inject faults into your system (Chaos ingeneering)
...
- **Anti-entropy** deals with resolving conflicts in the application state (in the actual payload, from appliction point of view)
...
- The minimal amount of data to be moved is K/N, where K is the number of keys (and N nodes)
- With K/N movements, the data only moves from the old nodes to the new one

### Consistent hashing

- **Consistent hashing** allows to achieve re-sharding in a K/N movements
...
- The node is reponsible for all the keys coming before it
...
- In case of a node crash, you lose multiple virtual nodes, however, Amazon actually says it's a feature, since this allows to spread the network trafic moving the data upon re-sharding
...
- In a lots of cases, the computation itself is conceptually very simple, but a lot of effort needs to be put into handling "distributed stuff"
...
- An upper bound of [a,b] in S is the least upper bound if u <= v fror each upper bound v of a and b
